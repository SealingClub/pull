...
module.exports = class Pull {
  constructor (github, { owner, repo, upstreamRepo, upstreamOwner, logger = console }, config) {
    this.github = github
    this.logger = logger
    ...
    this.config = Object.assign({}, value, { owner, repo, upstreamRepo, upstreamOwner, repoPath: `${owner}/${repo}` })
  ...
  async getUpstreamThanBehindHead(rule) {
    const { upstream, beforeHead } = rule
    if (!beforeHead) return upstream
    ...
    const branch = upstream.replace(new RegExp('^' + owner + ':'), '')
    const commitsWithBehind = new Map()

    const getBehindThanBase = (ref, base) => this.github.repos.compareCommits({
      owner: owner,
      repo: repo,
      head: ref,
      base: base,
    }).then(res => res.data.behind_by)
    const getBehindThanHead = (ref) => getBehindThanBase(ref, branch)

    if (beforeHead.byDays > 0) {
      const date = new Date()
      date.setDate(date.getDate() - beforeHead.byDays)
      const { data: [ { sha: commit } ] } = await this.github.repos.listCommits({
        owner: owner,
        repo: repo,
      ...
      const behind = await getBehindThanHead(commit)

      commitsWithBehind.set(behind, commit)
    }

    if (beforeHead.byPush > 0) {
      let events = []
      let req = await this.github.activity.listRepoEvents({
          owner: owner,
          repo: repo,
      })
      let pushCount = 1
      ...
      const postLoop = async () => {
        req = await this.github.getNextPage(req)
        pushCount += events.length
        const prev = events.pop()
        if (prev) previousEvent = prev
      }
      for (; pushCount < beforeHead.byPush; await postLoop()) {
        events = req.data.filter(i => 
          i.type === 'PushEvent' &&
          i.payload.ref.replace(/^refs\/heads\//, '') === branch
        )
        if (!this.github.hasNextPage(req)) {
          await setResult()
          break
        }
        if (!events.length) continue

        if (previousEvent) linearizationPushEvents(events, previousEvent)
        else linearizationPushEvents(events, previousEvent = events.shift())
        if (!events.length) continue

        if (pushCount + events.length >= beforeHead.byPush) {
          await setResult()
          break
        }
      }
      async function setResult() {
        const { payload: { head: commit } } = events[beforeHead.byPush - pushCount]
        const behind = await getBehindThanHead(commit)

        commitsWithBehind.set(behind, commit)
      }
      function linearizationPushEvents(events, prevEvent) {
        for (let i = events.length - 1; i > 0; i--) {
          const prev = events[i - 1].payload
          const curr = events[i].payload

          if (prev.before === curr.head) continue
          events.splice(i, 1)
          i++
        }

        if (!events.length) return
        if (prevEvent && prevEvent.payload.before !== events[0].payload.head) events.shift()
      }
    }

    if (beforeHead.byTilde > 0) {
      const { data: { sha : commit } } = await this.github.repos.getCommit({
        owner: owner,
        repo: repo,
        ref: branch + '~' + beforeHead.byTilde
      })
      commitsWithBehind.set(beforeHead.byTilde, commit)
    }

    const commitKey = beforeHead.useOlder 
      ? Math.max(...commitsWithBehind.keys())
      : Math.min(...commitsWithBehind.keys())

    return commitsWithBehind.get(commitKey)
  }
  ...
  async routineCheck () {
    ...
    for (let i = 0; i < this.config.rules.length; i++) {
      ...
      const { base, assignees, reviewers } = rule
      const upstream = await this.getUpstreamThanBehindHead(rule)
      ...
      const openPR = await this.getOpenPR(base) // Get PR opened by pull[bot]
      if (openPR) {
        this.tryUpdatePR(openPR, base, upstream)
        this.logger.debug(`[${this.config.repoPath}] Found a PR from ${upstream} to ${base}`)
        await this.checkAutoMerge(openPR)
        continue
      }

      this.logger.info(`[${this.config.repoPath}] Creating PR from ${upstream} to pull/${base}`)
      await this.checkAutoMerge(await this.createPR(base, upstream, assignees, reviewers))
    }
  }

  async tryUpdatePR(incomingPR, base, upstream) {
    const { data: { behind_by, ahead_by, commits } } = await this.github.repos.compareCommits({
      owner: this.config.owner,
      repo: this.config.repo,
      head: upstream,
      base: `pull/${base}`,
    })
    if (!behind_by && ahead_by) {
      this.logger.info(`[${this.config.repoPath}] the PR '${incomingPR.title}'#${incomingPR.number} hasn't handle, upgrading owned branch`)
      await this.createOrUpdateOwnedBranch(base, commits[commits.length - 1].sha)
      await this.github.pulls.update({
        owner: this.config.owner,
        repo: this.config.repo,
        pull_number: incomingPR.number,
  ...
  async checkAutoMerge (incomingPR, config = {}) {
    ...
    const rule = this.config.rules.filter(r => r.base.toLowerCase() === incomingPR.base.ref.toLowerCase())[0]
    ...
    if (rule && rule.mergeMethod !== 'none' &&
      ...
      incomingPR.user.login === 'pull-sealing-club-ver[bot]' &&
  ...
  async getOpenPR (base) {
    base = base.replace(`${this.config.owner}:`, '')
    const res = await this.github.issues.listForRepo({
      owner: this.config.owner,
      repo: this.config.repo,
      creator: 'pull-sealing-club-ver[bot]',
      per_page: 100
    })
    if (res.data.length > 0) {
      for (let i = 0; i < res.data.length; i++) {
        ...
        if (
          pr.data &&
          pr.data.user.login === 'pull-sealing-club-ver[bot]' &&
          pr.data.base.label.replace(`${this.config.owner}:`, '') === base &&
          pr.data.head.label.replace(`${this.config.owner}:`, '') === `pull/${base}`
        ) {
          return pr.data
        }
      }
  ...
  async createOrUpdateOwnedBranch(branch, ref) {
    const head = `pull/${branch}`
    const upstreamIsSha1 = /^[a-f0-9]{40}$/i.test(ref)
    if (!upstreamIsSha1) {
      const { data: { commit } } = await this.github.repos.getBranch({
        owner: this.config.upstreamOwner,
        repo: this.config.upstreamRepo,
        branch: ref.replace(new RegExp('^' + this.config.upstreamOwner + ':'), ''),
      })
      ref = commit.sha
    }
    try {
      ...
      if (e.message === 'Reference already exists') {
        this.logger.debug(`Updating ref for '${head}'`)
      ...
      else this.logger.warn(`Fail to touch branch '${head}'`)
    }
 
  }

  async createPR (base, upstream, assignees, reviewers) {
    await this.createOrUpdateOwnedBranch(base, upstream)
    try {
