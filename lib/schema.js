...
const fields = {
  ...
  behindHead: Joi.object({
    byDays: Joi.number().description('create PR using commits from HEAD {n} days ago'),
    byPush: Joi.number().description('create PR using the recent {n} pushed commits'),
    byTilde: Joi.number().description('create PR using commits from HEAD~{n}'),
    useOlder: Joi.boolean().description('If specify more than one, the oldest commit is used').default(false),
  })
  .description('Create PR from how far away the HEAD is'),
}
